...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    return res.status(500).json({ error: "Falta OPENAI_API_KEY en las variables de entorno" });
  }
...
Tipo de tirada: ${drawType || "desconocido"}
Cartas extraídas: ${cardsDescription}
...
Termina con un consejo práctico concreto.
    ...
    console.error(err);
    return res.status(500).json({ error: "Error interno en el oráculo" });
  }
}
